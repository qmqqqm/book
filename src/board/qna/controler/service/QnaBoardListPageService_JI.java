...
public class QnaBoardListPageService_JI {
		//field
		//필수변수
		private int total;			//전체 게시물수<-DB의 article 테이블의 전체 record수 
		private int currentPage;	//현재 페이지<-user가 보고싶은 페이지
		
		private List<QnaListDTO_JI> content;//user가 보고싶은 페이지에 속한 특정범위의 게시물조회결과 
		...
		//construcotr
		public QnaBoardListPageService_JI() {}
		 ...
		 */
		public QnaBoardListPageService_JI(int total, int currentPage, int size, List<QnaListDTO_JI> content) {
		...
		public List<QnaListDTO_JI> getContent() {
			return content;
		}
