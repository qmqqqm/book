...
public class OrderListArticleService_jg {

	//p651 13라인
	private OrderDAO articleDao = new OrderDAO();
	private int size = 2;  //한 페이지에 출력할 게시물수
	
	
	//(페이지별로 원하는 특정 범위의)글목록 조회 -  p651 19라인
		public OrderArticlePage_jg getArticlePage(int pageNum) {
			Connection conn = null;
			try {
				List<OrderArticle> content = null;
				...
				//p651 21라인
				return new OrderArticlePage_jg(total, pageNum, size, content);
	...
	public List<OrderArticle> getArticlePage2(int pageNum) {
		List<OrderArticle> content = null;
		Connection conn = null;
		try {
			//1page 글번호 1,2,3 -> 0,1,2
