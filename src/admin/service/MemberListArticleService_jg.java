...
public class MemberListArticleService_jg {
	...
	//(페이지별로 원하는 특정 범위의)글목록 조회 -  p651 19라인
		public MemberArticlePage_jg getArticlePage(int pageNum) {
			Connection conn = null;
			try {
				List<MemberArticle> content = null;
				...
				//p651 21라인
				return new MemberArticlePage_jg(total, pageNum, size, content);
	...
	public List<MemberArticle> getArticlePage2(int pageNum) {
		List<MemberArticle> content = null;
		Connection conn = null;
		try {
			//1page 글번호 1,2,3 -> 0,1,2
