...
import admin.service.StaffModifyArticleService_jg;
import admin.service.StaffModifyRequest_jg;
import admin.service.StaffReadService_jg;
...
public class adm_staffModifyArticleHandler_jg implements CommandHandler_Mj {

	private static final String FORM_VIEW = "../admin/adm_StaffmodifyForm.jsp";

	// 글상세보기를 위한 ReadArticleService 참조변수선언
	private StaffReadService_jg readService = new StaffReadService_jg();

	// 글수정을 위한 ModifyArticleService 참조변수선언
	private StaffModifyArticleService_jg modifyService = new StaffModifyArticleService_jg();
   ...
   StaffModifyRequest_jg modifyReq = new StaffModifyRequest_jg(authUser.getId(),no,id,pwd);
   ...
   modifyService.modify(modifyReq,no,id,pwd,hp);
   ...
   //4.View
   return "../admin/adm_ModifySuccess_jg.jsp";
	...
	private String processForm(HttpServletRequest request, HttpServletResponse response) throws IOException, ArticleNotFoundException_jg {
		...
		System.out.println("GET방식 pageNo="+pageNo);
			...
			//p670 49라인
			//(session)로그인한 user의 id(파라미터로받은),글번호,제목,내용
			//Model로 넘긴다
			StaffModifyRequest_jg modifyReq = new StaffModifyRequest_jg(authUser.getId(),
					no,
					articleData.getArticle().getId(),
					articleData.getArticle().getPwd(),
					articleData.getArticle().getHp());
	...
	private boolean canModify(User_jg authUser, ArticleData articleData) {
