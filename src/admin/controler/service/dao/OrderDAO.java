	...
	//접근제한자   속성  리턴유형  메서드명(매개변수리스트) {}
		...
		public int delete(Connection conn,int no) throws SQLException {
		      ...
		      try {
		         ...
		         sql = "delete  from order_jg where ord_No=?";
		         stmt = conn.prepareStatement(sql);
		         stmt.setInt(1, no);   //?세팅   
		         return stmt.executeUpdate();
		...
		public int selectcountid(Connection conn) throws SQLException {
		      String sql = null;
		      PreparedStatement stmt = null;
		      ResultSet rs = null;
		      ...
		      int sum=0;
		      try {
		         sql = "select count,book_id  from order_jg ";
		         stmt = conn.prepareStatement(sql);
		         rs = stmt.executeQuery();
		         while(rs.next()) { //특정 회원의 정보가 있으면
						...
						price = bookDao.selectprice(conn,rs.getInt("book_id"));
						sum += count*price;						
		...
		public int selectidcount(Connection conn, int no) throws SQLException {
		      String sql = null;
		      PreparedStatement stmt = null;
		      ResultSet rs = null;
		      BookDAO bookDao = new BookDAO();
		      ...
		      int sum=0;
		      try {
		         sql = "select count,book_id  from order_jg where ord_no=? ";
		         stmt = conn.prepareStatement(sql);
		         stmt.setInt(1, no);	//?세팅	
		         rs = stmt.executeQuery();
		         if(rs.next()) { //특정 회원의 정보가 있으면
						...
						//db에서 조회한 컬럼의 값을   => rs.get데이터타입(String 컬럼명)
						//매개변수를 가진 constructor를 이용하여 MemberDTO에 set
						sum = bookDao.updatecount(conn,rs.getInt("count"),rs.getInt("book_id"));
					}
		         return sum;
		      }finally{ //resource 반납
		         JdbcUtil_Mj.close(stmt);
