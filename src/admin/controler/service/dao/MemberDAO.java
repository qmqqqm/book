	...
	//접근제한자   속성  리턴유형  메서드명(매개변수리스트) {}
		...
		public  MemberDTO  selectById(Connection conn, String id)
		 throws SQLException {
			...
			ResultSet rs = null;
			MemberDTO member = null; //개인 회원의 다양한 정보를 담기위한 객체변수 선언
			try {
				sql = "SELECT  mem_No, mem_Id, mem_Pwd,mem_Name,mem_Birth,mem_Gender,mem_Hp,mem_Point,mem_Grade " + 
					  " FROM   member_Mj " + 
					  " WHERE  mem_Id=?";
				...
				if(rs.next()) { //특정 회원의 정보가 있으면
					...
					member = new MemberDTO(rs.getInt("mem_No"), 
											rs.getString("mem_Id"), 
		...
		public List<MemberArticle> select(Connection conn, int startRow, int size) throws SQLException {
			...
			try {
				...
				List<MemberArticle>  result  = new ArrayList<MemberArticle>();
		...
		private MemberArticle convertArticle(ResultSet rs) throws SQLException {
			//참고  rs.getXxx("")
			...
			//p647 27라인
			return 
				new MemberArticle(rs.getInt("mem_no"), rs.getString("mem_Id"), rs.getString("mem_Pwd"), rs.getString("mem_Name"), rs.getDate("mem_Birth"), rs.getString("mem_Gender"), rs.getString("mem_hp"), rs.getInt("mem_Point"), rs.getInt("mem_Grade"));
		}//convertArticle()

		public MemberArticle selectById(Connection conn, int articleNum) throws SQLException {
		      String sql = "SELECT mem_No, mem_Id, mem_Pwd,mem_Name,mem_Birth,mem_Gender,mem_Hp,mem_Point,mem_Grade "+
		                "FROM member_Mj "+
		                "WHERE mem_No=?";
		      PreparedStatement stmt = null;
		      ResultSet rs = null;
		      MemberArticle article = null;
		      try {
		         //쿼리실행-객체준비,실행
		         stmt = conn.prepareStatement(sql);
		         //?셋팅
		...
		public int update(Connection conn, int articleNumber, String id, String pwd,String name,String gender, String hp ,int point,int grade) throws SQLException {
			
		      String sql = null;
		      PreparedStatement stmt = null;
		      try {
		         sql = "update member_Mj set mem_Id=?,mem_pwd=?,mem_name=?,mem_gender=?,mem_hp=?,mem_point=?,mem_grade=? "
		         		+ "where mem_No =?";
		         stmt = conn.prepareStatement(sql);
		         stmt.setString(1, id);   //?세팅      
		         stmt.setString(2, pwd);   //?세팅      
		         stmt.setString(3, name);   //?세팅      
		         stmt.setString(4, gender);   //?세팅      
		         stmt.setString(5, hp);   //?세팅      
		         stmt.setInt(6, point);   //?세팅      
		         stmt.setInt(7, grade);   //?세팅      
		         stmt.setInt(8, articleNumber);   //?세팅      
