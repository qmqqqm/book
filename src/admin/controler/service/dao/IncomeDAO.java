...
public class IncomeDAO {
	//접근제한자   속성  리턴유형  메서드명(매개변수리스트) {}
		...
		public int update(Connection conn, int no) throws SQLException {
		      ...
		      PreparedStatement stmt = null;
		      try {
		         sql = "update income_Jg set income = income+? ";
		         stmt = conn.prepareStatement(sql);
		         stmt.setInt(1, no);   //?세팅      
		...
		public int selectincome(Connection conn) throws SQLException {
		      String sql = null;
		      PreparedStatement stmt = null;
		      ResultSet rs = null;
		      ...
		      int price=0;
		      try {
		         sql = "select income from income_jg ";
		         stmt = conn.prepareStatement(sql);
		         rs = stmt.executeQuery();
		         if(rs.next()) { //특정 회원의 정보가 있으면
		        	 price = rs.getInt("income");
					}
		         return price;
		      }finally{ //resource 반납
		         JdbcUtil_Mj.close(stmt);
