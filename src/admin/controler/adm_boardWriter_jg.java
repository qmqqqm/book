...
import admin.service.BoardWriterService;
import admin.service.User_jg;
import mvc.command.CommandHandler_Mj;

//글쓰기폼 보여주기 요청(GET방식) + 글쓰기처리 요청(POST방식)
public class adm_boardWriter_jg implements CommandHandler_Mj {

	private static final String FORM_VIEW = "../admin/adm_WriteForm_jg.jsp";
	BoardWriterService qnaWriteService =  new BoardWriterService(); 
	
	@Override
	public String process(HttpServletRequest request, HttpServletResponse response) throws Exception {
		  System.out.println("BoardWriteControler 의 process()진입/method방식="+request.getMethod());
		//METHOD방식을 구분해서
	...
	private String processSubmit(HttpServletRequest request, HttpServletResponse response) {
	  ...
	  QnaWriteRequest_jg qnaWriteReq = createWriteRequest(user,request);	//user id, title, content 저장
	  ...
	  //4.View
	  return "../admin/adm_WriteSuccess_jg.jsp";
	  
	}//processSubmit()
	
	//p641 53라인
	private QnaWriteRequest_jg createWriteRequest(User_jg user,HttpServletRequest request) {
	   
	   return 
	   new QnaWriteRequest_jg(user.getId(),
