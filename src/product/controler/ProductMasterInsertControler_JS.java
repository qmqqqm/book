...
import product.controler.service.ProductMasterInsertService;


//이 클래스 모든 상품의 모든 정보를 가져올 클래스이다
//이 컨트롤러는   /product/searchAll.book 요청이오면  호출되는 담당컨트롤러이다
//비즈니스로직실행 <->ProductService2<->ProductDAO2<->db
public class ProductMasterInsertControler_JS implements CommandHandler_Mj {
	
		private ProductMasterInsertService productMasterInsertService = new ProductMasterInsertService();
	 ...
	 * C:\Users\phy85\OneDrive\문서\카카오톡 받은 파일\0312\JBKBOOK\WebContent\img
	 */     int size=1024*1024*10; 
	     ...
	     String fname=multi.getFilesystemName("fname_insert"); 
	...
	System.out.println("discount_rate="+discount_rate);
			...
			ProductDTO product_master_insert = new ProductDTO(
					book_kind,
					book_title,//
			...
			System.out.println("나는 서비스에 가는중"+product_master_insert);
				 productMasterInsertService.getProductList(product_master_insert);
				
				 System.out.println("나는 서비스에 갔다왔어");
				
				//3.모델- session or request
				System.out.println("ProductMasterInsertControler_JS -gomain");
				request.setAttribute("product_master_insert",product_master_insert);
				System.out.println("request.getAttribute()="+request.getAttribute("product_master_insert"));
		...
		private String processForm(HttpServletRequest request, HttpServletResponse response) {
			return "../product/product_Insert_JS.jsp";
		}
}
