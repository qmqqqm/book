...
<link rel="stylesheet" href="<%=request.getContextPath()%>/css/mem_Join_Mj.css">
<link rel="shortcut icon" href="<%=request.getContextPath()%>/img/pabi/pabi.ico" type="image/x-icon"/>
<script src="http://code.jquery.com/jquery-latest.min.js"></script>
<script src="<%=request.getContextPath()%>/js/mem_Join_Mj.js"></script>
<script>
...
$(function(){
	$("#btnDuplicate").click(function(){
		...
		//id가  mId인 요소의 value를 가져와   _id변수에 저장
		var _id=$("#id").val();
...
/* 아작스을 이용해 서버에 유져가 입력한 아이디 확인후 상황에 맞는 메세지 출력  */
		$.ajax({
			url:  "${contextPath}/member/idcheck.book", 
			type:	"post",
			data:   {id:_id},
			dataType: "text",
			async : false,
			success:				
				...
				/* 아이디가 사용가능하다면 */
				 if(data=='usable'){
      	   $('#result1').text("사용 가능한 ID입니다.");
      	   
      	   
      	   //아이디가 사용중이라면 
         }else{
      	   $('#result1').text("이미 사용중인 ID입니다.");
         }						
				},
			error:	
				function(xhr, textStatus, errorThrown){ 
					alert(textStatus+" error가 발생했어요");
				}
		});//$.ajax({
	});//#btn1클릭시	
});//jQuery
...
<br>
<hr/>
<form name="frm" id="frm" action="<%=request.getContextPath()%>/member/join.book" method="post">
<table >
	<tr>
        <td colspan="2" class="agree">약관동의</td>
    </tr>
    <tr>
        <td colspan="2" class="agree"><textarea rows="20" cols="100" readonly="readonly">
제1조 (목적)
본 약관은 JBK(이하 '회사')가 JBKBOOK 사이트(이하 ‘사이트’)를 통하여 
인터넷상에서 제공하는 서비스(이하 ‘서비스’)를 이용하는 고객 (이하 '고객')간의 권리와 
의무 및 책임 등 기타 제반사항을 규정함을 목적으로 합니다.

제2조 (용어의 정의)
이 약관에서 사용하는 용어의 정의는 다음과 같습니다.
가. '회원'이라 함은 이 약관에 동의하고 회원가입을 통하여 이용자ID(고유번호)와 비밀번호를 발급받은 자로서, 
회사가 제공하는 서비스를 이용할 수 있는 이용자를 말합니다.
나. '이용자ID'라 함은 회원의 식별 및 서비스 이용을 위하여 
회원의 신청에 따라 회사가 회원 별로 부여하는 고유한 문자와 숫자의 조합을 말합니다.
다. '비밀번호'라 함은 이용자ID로 식별되는 회원의 본인 여부를 검증하기 위하여 
회원이 설정하여 회사에 등록한 고유의 문자와 숫자의 조합을 말합니다.
...
부칙 (시행일)
본 약관은 2021년 3월 4일부터 적용합니다.
        
        </textarea>   </td>
    </tr>
    	<tr>
        <td colspan="2" class="agree"><input type="checkbox" id="agree" name="agree"><label class="agr" for="agree">동의</label></td>
    </tr>
    <tr>
        <td colspan="2" class="mem">회원가입</td>
    </tr>
    <tr>    
        <td class="index">ID</td>
        <td><input class="intext" type="text" name="id" id="id" value="${param.id}" placeholder="아이디를 입력하세요">
    ...
    <tr>
        ...
        <td><input class="intext" type="password" name="pwd" id="pwd" placeholder="비밀번호를 입력하세요">
        <c:if test="${errors.pwd}"> 암호 를 입력하세요</c:if>
        </td>    	
    </tr>
    <tr>
        <td class="index">비밀번호 확인</td>
        <td><input class="intext" type="password" name="confirmPwd" id="confirmPwd" placeholder="비밀번호를 다시 입력하세요">
        <c:if test="${errors.ConfirmPwd}"> 암호를 한번더 입력하세요</c:if>
    	<c:if test="${errors.notMatch}"> 암호 확인이 일치하지 않습니다</c:if>
    	</td>    	
    </tr>
    <tr>
        <td class="index">이름</td>
        <td><input class="intext" type="text" name="name" id="name" value="${param.name}" placeholder="이름를 입력하세요">
        <c:if test="${errors.name}"> 이름를 입력하세요</c:if>
        </td>        
    </tr>
    <tr>
        <td class="index">생년월일</td>
        <td><input class="intext" type="text" value="" name="birth" id="birth" placeholder="생년월일를 입력하세요"> </td>
    </tr>
    <tr>
        <td class="index">성별</td>
        <td>      <select class="gender" name="gender" id="gender" placeholder="성별를 입력하세요">
                    <option >선택</option>
                    <option >남자</option>
                    <option >여자</option>
    ...
    <tr>
        ...
        <td><input class="intext" type="text" value="" name="hp" id="hp" placeholder="핸드폰 번호를 입력하세요"></td>
    </tr>
    <tr>
        <td colspan="2" class="mem"><input class="joinButton" type="submit" value="회원가입"><input class="joinButton" type="reset" value="다시입력"></td>
    </tr>
</table>
</form>
</div>
