		...
		<table class="button">
			<tbody>
				<tr>
				  <c:if test="${!empty ADMIN}">
					<td class="button">
						<a href="../admin/boardWrite.book">글쓰기</a>
					</td>
				  </c:if>
				</tr>
			</tbody>
		...
		<caption>공지사항</caption>
			...
			<tbody>
				...
				<c:forEach var="article" items="${articlePage.content}"> 
					<tr>
						<td width="260"><a href="./boarddetail.book?no=${article.number}&pageNo=${articlePage.currentPage}"> ${article.title}</a></td>
				...
				<tr>
					<td class="pageNo">
						...
						<c:if test="${articlePage.startPage>5}">    
							<a href="<%=request.getContextPath()%>/admin/board.book?pageNo=${articlePage.startPage-5}"> &lt;&lt;prev </a>				
						</c:if>
						...
						<c:forEach begin="${articlePage.startPage}" 
						           ...
						           var="pNo">	
							<a href="<%=request.getContextPath()%>/admin/board.book?pageNo=${pNo}">[${pNo}]</a>
						</c:forEach>
						...
						<c:if test="${articlePage.endPage<articlePage.totalPages}">   
							<a href="<%=request.getContextPath()%>/admin/board.book?pageNo=${articlePage.startPage+5}"> next&gt;&gt; </a>
						</c:if> 
					</td>
				</tr>
			</tbody>
