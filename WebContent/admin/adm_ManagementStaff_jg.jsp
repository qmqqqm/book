			...
			<thead>
				<tr>
						<th width="60">번호(article_no)</th>
							<th width="260">아이디(id)</th>
							<th width="120">비밀번호(pwd)</th>
			...
			<tbody>
				...
				<c:forEach var="article" items="${articlePage.content}">
							<tr>
								...
								<td width="260"><a href="./staffdetail.book?no=${article.number}&pageNo=${articlePage.currentPage}">${article.id}</a></td>
						...
						--%>
								<td width="120">${article.pwd}</td>
								<td width="60">${article.hp}</td>
							</tr>
						</c:forEach>
				<%-- 반복끝 -------------%>
			</tbody>	
				...
				<tr>
					<td style="text-align:center;">
						...
						<c:if test="${articlePage.startPage>5}">    
							<a href="<%=request.getContextPath()%>/admin/staff.book?pageNo=${articlePage.startPage-5}"> &lt;&lt;prev </a>				
						</c:if>
						...
						<c:forEach begin="${articlePage.startPage}" 
						           ...
						           var="pNo">	
							<a href="<%=request.getContextPath()%>/admin/staff.book?pageNo=${pNo}">[${pNo}]</a>
						</c:forEach>
						...
						<c:if test="${articlePage.endPage<articlePage.totalPages}">   
							<a href="<%=request.getContextPath()%>/admin/staff.book?pageNo=${articlePage.startPage+5}"> next&gt;&gt; </a>
						</c:if> 
					</td>
				</tr>
			</tbody>
